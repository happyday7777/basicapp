...
export default function GalleryCard({item}) {
    // const item = item.item;
        ...
        let sptags = item.galSearchKeyword
            ? (item.galSearchKeyword.includes(',')
  ...
  return (
    <div className="max-w-sm rounded overflow-hidden shadow-lg">
        <img className="h-48 w-full "
            src={item.galWebImageUrl.includes('http:') ? item.galWebImageUrl.replace('http:', 'https:') : `https:${item.galWebImageUrl}`}
            alt={item.galTitle} />
